...
def populate_table_specific_attribute_df(d1,d2):
    ...
    write_excel('C:\\app\\modelling\\staff\\test2_out.xlsx','merged_df',merged_df)
...
model_template = 'C:\\app\\modelling\\staff\\Copy of TDC_Mapping_Workbook_Template Adviser V14.xlsx'
...
main_model_df = pd.read_excel(ref_file, sheet_name='model',header=0) # This should be changed to ref.advisor_model
main_model_df = main_model_df.loc[(main_model_df['phase (1/2)'] == 1) & (main_model_df['modelled'] == 'y')]
main_model_df = main_model_df[['column_name','silver','gold','mv','pk','source data type','transformed_data_type','modelled']]

main_model_df['source data type'] = main_model_df['transformed_data_type'].combine_first(main_model_df['source data type'])
...
main_gold_df = main_model_df[['column_name','gold','source data type','pk','modelled']]
#print(main_gold_df.to_markdown())
...
#populate_pks(silver_sql_file,silver_pk_df)  This is a erstudio model so using populate_pks_ers
...
#print(gold_model_df.to_markdown())
...
cons_df = populate_pks_ers(gold_pk_df)
#gold_ddl = populate_ddl(gold_sql_file,gold_model_df)
gold_ddl = populate_ddl_ers(gold_sql_file,gold_model_df,cons_df)

populate_table_excel(gold_model_df,ref_file,format_outfile,model_template,excel_dir+'gold\\') # We need the gold version for this
